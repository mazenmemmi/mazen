...
echo '<div id="menu">';
$sqqql="SELECT * FROM m_sousrubrique where rubID=".$rmyrub." ORDER BY sruborder;";
$dsql=mysqli_query($mydbobj,$sqqql);

while ($dsq=mysqli_fetch_array($dsql))
{
        echo'<dl><dt><a href="#">'.$dsq["srubLibelle"].'</a></dt>';
		$red="select * from documents where srubID=".$dsq["srubID"]." ORDER BY docorder;";
		  ...
		  }else
		  {
			  echo '<dd><ul>';
	           ...
	           while($red1=mysqli_fetch_array($Rre))
                  {
	                echo'<li><a href="#">'.$red1["doctitle"].'</a></li>';
	               }
              echo '</ul></dd>';
...
<tr>';
echo '<td><a href="#"><img src="images/face.png" width="50" height="50" /></a></td>&nbsp;<td><a href="#"><img src="images/google.png" width="50" height="50" /></a></td>';
echo  '</tr></table>';
echo"</td>";
...
$RSdoc=NULL;*/
   ...
   $sqldoc="SELECT * FROM m_sousrubrique where rubID=".$rmyrub." ORDER BY sruborder;";
    ...
    if ($con<=4)
	 {
          echo '<td class="doctitle" width="140" height="150"  valign="bottom"  align="center" no-repeat; background-size:cover;"><a href=doc.php?mcat='.$rmycat.'&mrub='.$datas["rubID"].'&msrub='.$datas["srubID"].'>'.$datas['srubLibelle'].'</a></td>';
		  $con=$con+1;
	}//echo '<br><hr color="#FF6600" size="1"></td></tr>'; 
    else
	{
	 echo '</tr><tr><td class="doctitle" width="140" height="150" valign="bottom"  align="center" no-repeat; background-size:cover;"><a href=doc.php?mcat='.$rmycat.'&mrub='.$datas["rubID"].'&msrub='.$datas["srubID"].'>'.$datas['srubLibelle'].'</a></td>';	//<br>
	 $con=1;
	}
	 } 
...
$image=substr ($datas["docimage"],3,strlen($datas["docimage"]));
      ...
      {
       echo'<td align="center" >'.$datas["docbody"].'</td>';
        ...
        else
      {
	   if ($con<=4)
	{
		echo '<td class=doctitle width="140" height="150" valign="bottom"  align="center" style="background:url('.$image.') no-repeat; background-size:cover;"><a href=doc.php?docID='.$datas["docID"]."&mcat=".$rmycat."&mrub=".$rmyrub."&msrub=".$rmysrub.'>'.$datas["doctitle"].'</a></td>';
	}
	else 
	{
		echo '</tr><tr  ><td class=doctitle width="140" height="150" valign="bottom"  align="center" style="background:url('.$image.') no-repeat; background-size:cover;" ><a href=doc.php?docID='.$datas["docID"]."&mcat=".$rmycat."&mrub=".$rmyrub."&msrub=".$rmysrub.'>'.$datas["doctitle"].'</a></td>';//<br>
		$con=1;
	}
      } 
}
